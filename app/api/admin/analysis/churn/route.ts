...
import { getFeatureChurnReport } from "@/lib/admin/analysis/service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await getFeatureChurnReport({
    ...
    return NextResponse.json(
      { success: false, error: error?.message || "Failed to load churn report" },
      { status },
    );
  }
}
