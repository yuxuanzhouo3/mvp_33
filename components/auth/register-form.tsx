    ...
    try {
      if (provider === 'google' && !IS_DOMESTIC_VERSION && isAndroidWebView()) {
        ...
        await signOutGoogle().catch((error) => {
          console.warn('[REGISTER FORM] Native Google signOut preflight failed:', error)
        })
        ...
        try {
          ...
          console.warn('[REGISTER FORM] Native Google login device record failed:', deviceError)
        }
